...
const getFullValue = (half: number): number => {
  return parseInt(`${half}${half}`);
};
const getInvalidSumForRange = (start: number, end: number): number => {
  ...
  for (let half = firstHalf; ; half++) {
    const fullValue = getFullValue(half);
...
const total = ranges
  ...
  .reduce((acc, [start, end]) => acc + getInvalidSumForRange(start, end), 0);
